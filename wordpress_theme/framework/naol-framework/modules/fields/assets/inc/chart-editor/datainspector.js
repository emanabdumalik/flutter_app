    ...
    var Inspector = /** @class */ (function () {
         ...
         * @return {boolean}
         */
        Inspector.showIfNode = function (part) { return part instanceof rimes.Node; };
        /**
         * This predicate function can be used as a value for the `show` option for properties.
         ...
         * @return {boolean}
         */
        Inspector.showIfLink = function (part) { return part instanceof rimes.Link; };
        /**
         * This predicate function can be used as a value for the `show` option for properties.
         ...
         * @return {boolean}
         */
        Inspector.showIfGroup = function (part) { return part instanceof rimes.Group; };
        /**
         * This predicate function can be used as a value for the `show` option for properties.
        ...
        Inspector.prototype.inspectObject = function (object) {
            ...
            if (!inspectedObjects && inspectedObject) {
                inspectedObjects = new rimes.Set();
            ...
            if (inspectedObjects) {
                ...
                mainDiv.innerHTML = '';
                var shared = new rimes.Map(); // for properties that the nodes have in common
                var properties = new rimes.Map(); // for adding properties
                var all = new rimes.Map(); // used later to prevent changing properties when unneeded
                ...
                var data = (inspectedObject instanceof rimes.Part) ? inspectedObject.data : inspectedObject;
                ...
                while (it.next() && (this._showLimit < 1 || nodecount <= this._showLimit)) { // grabs all the properties from the other selected objects
                    ...
                    if (inspectedObject) {
                        ...
                        data = (inspectedObject instanceof rimes.Part) ? inspectedObject.data : inspectedObject;
                        if (data) {
        ...
        Inspector.prototype.convertToString = function (x) {
            ...
            if (x === null)
                return 'null';
            if (x instanceof rimes.Point)
                return rimes.Point.stringify(x);
            if (x instanceof rimes.Size)
                return rimes.Size.stringify(x);
            if (x instanceof rimes.Rect)
                return rimes.Rect.stringify(x);
            if (x instanceof rimes.Spot)
                return rimes.Spot.stringify(x);
            if (x instanceof rimes.Margin)
                return rimes.Margin.stringify(x);
            if (x instanceof rimes.List)
        ...
        Inspector.prototype.updateAllHTML = function () {
            ...
            var isPart = this._inspectedObject instanceof rimes.Part;
        ...
        Inspector.prototype.parseValue = function (decProp, value, input, oldval) {
            ...
            if (type === '') {
                if (typeof oldval === 'boolean')
                    ...
                    type = 'number';
                else if (oldval instanceof rimes.Point)
                    type = 'point';
                else if (oldval instanceof rimes.Size)
                    type = 'size';
                else if (oldval instanceof rimes.Rect)
                    type = 'rect';
                else if (oldval instanceof rimes.Spot)
                    type = 'spot';
                else if (oldval instanceof rimes.Margin)
                    type = 'margin';
            }
            ...
            switch (type) {
                ...
                case 'arrayofnumber':
                    ...
                    break;
                case 'point':
                    value = rimes.Point.parse(value);
                    break;
                case 'size':
                    value = rimes.Size.parse(value);
                    break;
                case 'rect':
                    value = rimes.Rect.parse(value);
                    break;
                case 'spot':
                    value = rimes.Spot.parse(value);
                    break;
                case 'margin':
                    value = rimes.Margin.parse(value);
                    break;
                case 'checkbox':
                    value = input.checked;
                    break;
            ...
            for (var name_5 in inspectedProps) {
                ...
                if (change) { // only change properties it needs to change instead all of them
                    for (var i = 0; i < diagram.selection.count; i++) {
                        it.next();
                        var isPart = it.value instanceof rimes.Part;
        ...
        Inspector.prototype.updateAllProperties = function () {
            var inspectedProps = this.inspectedProperties;
            var diagram = this._diagram;
            var isPart = this.inspectedObject instanceof rimes.Part;
