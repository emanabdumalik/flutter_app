 ...
 */
if ( ! class_exists( 'OT_Template_Meta_Box' ) ) {

  class OT_Template_Meta_Box {
    ...
    function __construct( $meta_box ) {
      ...
      global $ot_template_meta_boxes;

      if ( ! isset( $ot_template_meta_boxes ) ) {
        $ot_template_meta_boxes = array();
      }

      $ot_template_meta_boxes[] = $meta_box;

      $this->meta_box = $meta_box;

      add_action( 'add_meta_boxes', array( $this, 'add_meta_boxes' ) );

      add_action( 'save_post_cellular-template', array( $this, 'save_meta_box' ), 1, 2 );

      //add_action( 'update_post', 'flush_rewrite_rules', 1, 2 );

      // add_action( 'admin_notices', array( $this,'bstw_multiple_notice' ));
    ...
    function build_meta_box( $post, $metabox ) {
      ...
      echo '<div class="ot-metabox-wrapper">';
        ...
        foreach ( $this->meta_box['fields'] as $field ) {
          ...
          //echo apply_filters('harif_meta_validation',$field['id'],$_args['field_value'],$post);
      ...
      foreach ( $this->meta_box['fields'] as $field ) {
        ...
        if ( isset( $_POST[$field['id']] ) ) {
          ...
          if ( $field['type'] == 'javascript' ) {
            ...
            if ( '' !== $new ) {
              cellular_widgets_update_widget_js($post_object->ID,$new);
          ...
          if ( $field['type'] == 'css' ) {
            ...
            if ( '' !== $new ) {
              ...
              ot_insert_css_with_markers( $field['id'], $new, true );
                ...
                cellular_widgets_update_widget_css($post_object->ID,$new);
 ...
 * @since    2.0
 */
if ( ! function_exists( 'ot_register_widget_meta_box' ) ) {

  function ot_register_template_meta_box( $args ) {
    if ( ! $args )
      return;
      
    $ot_template_meta_box = new OT_Template_Meta_Box( $args );
  }
