<?php
/**
 * Plugin Name: CellularBuilder
 * Plugin URI:  http://themers.com/plugins/cellular-builder
 * Description: Drag and drop visual builder for wordpress
 * Version:     1.0
 * Author:      Rimes Gold
 * Author URI:  http://themers.com/author/rimes
 * License:     GPLv3
 * Text Domain: cellular-builder
 */
...
require_once plugin_dir_path(__FILE__) . 'inc/cellular-templates.php';
 ...
 */
function siteorigin_panels_admin_front_enqueue_scripts( $prefix = '', $force = false ) {
	// Media is required for row styles
	wp_enqueue_media();
	wp_enqueue_script( 'so-panels-admin-front', plugin_dir_url(__FILE__) . 'js/siteorigin-panels' . SITEORIGIN_PANELS_VERSION_SUFFIX . SITEORIGIN_PANELS_JS_SUFFIX . '-front.js', array( 'jquery', 'jquery-ui-resizable', 'jquery-ui-sortable', 'jquery-ui-draggable', 'underscore', 'backbone', 'plupload', 'plupload-all' ), SITEORIGIN_PANELS_VERSION, true );
	//wp_enqueue_script( 'so-panels-admin-front', plugin_dir_url(__FILE__) . 'js/siteorigin-panels' . SITEORIGIN_PANELS_VERSION_SUFFIX . SITEORIGIN_PANELS_JS_SUFFIX . '-front.js', array(  ), SITEORIGIN_PANELS_VERSION, true );




	add_action( 'wp_footer', 'siteorigin_panels_front_js_templates' );
	...
	wp_localize_script( 'so-panels-admin-front', 'panelsOptions', array(
	...
	do_action( 'siteorigin_panel_front_enqueue_admin_scripts' );
...
function siteorigin_panels_front_js_templates(){
	include plugin_dir_path(__FILE__).'tpl/js-front-templates.php';
}
 ...
 */
function siteorigin_panels_admin_front_enqueue_styles( $prefix = '', $force = false ) {
	wp_enqueue_style( 'so-panels-admin', plugin_dir_url(__FILE__) . 'css/admin-front.css', array( 'wp-color-picker' ), SITEORIGIN_PANELS_VERSION );
...
function siteorigin_panels_filter_content( $content ) {
	global $post;
	define('SOW_BUNDLE_VERSION', '1.6.2');
...
// Allow JS suffix to be pre-set
	...
	if ( !empty( $panels_data ) ) {
		...
		if ( !empty( $panel_content ) ) {
			if(isset($_GET['siteorigin_panels_live_editor']) && $_GET['siteorigin_panels_live_editor'] == true && is_user_logged_in()) {
				...
				siteorigin_panels_admin_front_enqueue_scripts();
				siteorigin_panels_admin_front_enqueue_styles();
			}
	...
	//include plugin_dir_path(__FILE__).'inc/admin-home-page.php';
