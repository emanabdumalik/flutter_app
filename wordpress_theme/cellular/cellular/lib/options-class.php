...
if ( ! class_exists( 'OT_Options_Meta_Box' ) ) {

	class OT_Options_Meta_Box {
		...
		function setup(){
			...
			$this->meta_box = apply_filters( 'option_additional_fields', array() );

			add_action('admin_menu', array($this,'options_admin_menu'));

			add_action( 'admin_init' , array( $this, 'save_meta_box' ) );
		}
		...
		function options_admin_menu(){
			add_theme_page(
				__( 'Meta Builder', 'siteorigin-panels' ),
				__( 'Meta', 'siteorigin-panels' ),
				'edit_theme_options',
				'harif_meta',
 ...
 * @since    2.0
 */
add_filter( 'option_additional_fields', 'mytheme_menu_item_additional_fields2' );
function mytheme_menu_item_additional_fields2( $fields ) {
	$fields = array(
		array(
			'id' => 'color',
			'name' => 'color',
...
$v = new OT_Options_Meta_Box;
add_action( 'init', array( $v, 'setup' ) );