(function($) {
    acf.fields.repeater = acf.field.extend({
        ...
        add: function($tr) {
            ...
            let element = $el.find('.siteorigin-page-builder-widget');
            let has_builder = element.length == 1;
            // remove clone class
            ...
            if (has_builder) {
                ...
                if (typeof jQuery.fn.soPanelsSetupBuilderWidget != 'undefined' && !jQuery('body').hasClass('wp-customizer')) {
                    $el.find('#' + the_id).soPanelsSetupBuilderWidget();
                    setTimeout(function() {
                        $el.find('.so-builder-container')[0].remove();
                    }, 10);
                }
            }
            // validation
