...
var WidgetUI = {
  ...
  CLASS_NAMES: {
    ...
    fontWeight: 'stylebot-border-style-control',
  ...
  createSizeControl: function(property) {
    ...
    this.selectize($select, {
      onValueChange: function(value) {
        var property = $select.prev().data('property');
        var unit = value;
$thevalue = $select.prev().val();
        if (parseFloat($thevalue)) {
          $thevalue += unit;
        }

        stylebot.style.apply(property, $thevalue);
      }
    });
  ...
  createFontFamilyControl: function(control) {
    ...
    $.post(
        ...
        cellularStylerOptions.ajaxurl,
        {action:'get_goole_fonts'},
        function(items) {
          items = $.parseJSON(items);
          var fontStack = items["fontStack"];
          if (!fontStack) {
            fontStack = x.DEFAULT_FONT_STACK;
          }

          localStorage.setItem("fontStack", fontStack);
          x.updateFontStack(fontStack);
        })
  ...
  this.selectize($select, {
    ...
    onValueChange: $.proxy(function(value) {
      if (value != '') {
          ...
          var fontStack = localStorage.getItem("fontStack");
fontStack   =   fontStack.split(',');
        if (fontStack.indexOf(value) != -1) {
          ...
          localStorage.setItem("fontStack", fontStack);
          x.updateFontStack(fontStack, value);
