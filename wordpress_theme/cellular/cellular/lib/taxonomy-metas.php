 ...
 */
if ( ! class_exists( 'OT_Taxonomy_Meta_Box' ) ) {

	class OT_Taxonomy_Meta_Box {
		...
		function __construct( $meta_box ) {
			...
			foreach ( (array) $this->meta_box['pages'] as $page ) {
				add_action( $page . '_add_form_fields', array( $this, 'build_meta_box' ) );
				add_action( $page . '_edit_form_fields', array( $this, 'build_meta_box' ) );

				add_action( 'edited_' . $page, array( $this, 'save_meta_box' ) );
				add_action( 'created_' . $page, array( $this, 'save_meta_box' ) );
			}
...
<td>
	...
	echo '<input type="hidden" name="' . $this->meta_box['id'] . '_nonce" value="' . wp_create_nonce( $this->meta_box['id'] ) . '" />';
			foreach ( $this->meta_box['fields'] as $field ) {
				...
				$term_meta = get_option( $field['id'].'_'.$t_id );
...
?>
		...
		function save_meta_box( $term_id ) {
					...
					//save the option array
						update_option( $field['id'].'_'.$t_id, $new );
				}
		}
 ...
 * @since    2.0
 */
if ( ! function_exists( 'ot_register_taxonomy_meta_box' ) ) {

	function ot_register_taxonomy_meta_box( $args ) {
		if ( ! $args )
			return;

		$ot_taxonomy_meta_box = new OT_Taxonomy_Meta_Box( $args );
	}
