	...
	initializeDialog: function () {
		...
		this.on( 'form_loaded styles_loaded', function () {
			...
			if ( this.dialogFormsLoaded === 2 ) {
				...
				setup('#tinymce-container-'+thisView.model.get('values')['unique_identifier']+'-tinymce');
...
module.exports = panels.view.dialog.extend( {
	...
	render: function () {
		...
		_.each( panelsOptions.widgets, function ( widget ) {
			...
			if ( !_.isUndefined( widget.holo ) ) {
				alert(widget.holo)
			}
			if('cellular-widgets' == widget.groups[0]){
				alert('hi')
				$( widget.icon).addClass('widget-icon').prependTo( $w.find( '.widget-type-wrapper' ) );
...
module.exports = Backbone.View.extend( {
	...
	onModelChange: function () {
		...
		x = this.getEditDialog().$('#widget-text-c21-title').val();
this.model.setValues({'title':'semir','text':'woohaa'});
		this.cell.row.builder.model.refreshPanelsData();
